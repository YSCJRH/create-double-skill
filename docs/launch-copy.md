# Launch Copy

为 `create-double-skill` 准备的 GitHub 展示文案。

## Recommended About

Build a private digital double from guided interviews and freeform self-description.

## About Variants

### Variant A

Build a private digital double from guided interviews and freeform self-description.

### Variant B

Local-first digital double builder for capturing voice, values, and decision style.

### Variant C

A high-signal, correction-friendly way to turn self-description into a private digital double.

## Recommended Topics

- `digital-double`
- `skill`
- `codex`
- `local-first`
- `personality-modeling`
- `prompt-engineering`

## Social Preview Copy

### Option A

- Title: `create-double-skill`
- Subtitle: `Build a private digital double`
- Footer: `Voice, values, and decision style`

### Option B

- Title: `分身.skill`
- Subtitle: `A local-first digital double builder`
- Footer: `Guided interviews + freeform self-description`

### Option C

- Title: `Not a life-log`
- Subtitle: `A high-signal digital double`
- Footer: `Private. editable. uncertainty-aware.`

## Recommended Social Preview Direction

- Main title: `create-double-skill`
- Supporting line: `Build a private digital double`
- Small line: `Capture how you speak, decide, and draw boundaries.`

## Tone Guidance

- Avoid sounding like a generic AI companion app.
- Emphasize `local-first`, `high-signal`, and `uncertainty-aware`.
- Prefer words like `voice`, `values`, `decision style`, and `corrections`.
- Avoid claims that imply memory simulation, surveillance-style data capture, or full personality cloning.
